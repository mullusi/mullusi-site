...
function validateRuntimeGateState() {
  ...
  for (const term of [
    ...
    "private_value_storage=outside_git",
    "secret_values=not_recorded",
    "host_addresses=not_recorded",
    "database_urls=not_recorded",
    "provider_values=not_recorded",
  ]) {
    if (!apiRuntimeManualEvidenceChecklist.includes(term)) {
  ...
  for (const { key } of [
    ...
    { key: "deploy_env_check_ready" },
    { key: "release_preflight_ready" },
    { key: "persistence_check_ready" },
    { key: "host_firewall_configured" },
    { key: "tls_certificate_ready" },
        ...
        : key === "managed_postgres_ready"
          ...
          : key === "schema_applied"
            ...
            : key === "production_secrets_stored"
              ...
              : key === "deploy_env_check_ready"
                ? `evidence_item=${key} state=Pass public_safe_ref=receipt://api-runtime/deploy-env-ready/2026-06-29`
                : key === "release_preflight_ready"
                  ? `evidence_item=${key} state=Pass public_safe_ref=receipt://api-runtime/release-preflight-ready/2026-06-29`
                  : key === "persistence_check_ready"
                    ...
                    : key === "host_firewall_configured"
                      ...
                      : key === "tls_certificate_ready"
                        ? `evidence_item=${key} state=Pass public_safe_ref=receipt://api-runtime/tls-ready/2026-06-29`
                      : key === "rollback_path_defined"
                        ? `evidence_item=${key} state=Pass public_safe_ref=site:ops/api-production-readiness-gate.md`
                        : key === "private_runtime_witness_ready"
                          ? `evidence_item=${key} state=Pass public_safe_ref=control-plane:receipt/runtime-witness-ready-2026-06-29`
                          : key === "dns_authority_ready"
                            ? `evidence_item=${key} state=Pass public_safe_ref=cloudflare:audit/dns-authority-2026-06-29`
        ...
        : key === "managed_postgres_ready"
          ...
          : key === "schema_applied"
            ...
            : key === "production_secrets_stored"
              ...
              : key === "deploy_env_check_ready"
                ? `"${key}": "receipt://api-runtime/deploy-env-ready/2026-06-29"`
                : key === "release_preflight_ready"
                  ? `"${key}": "receipt://api-runtime/release-preflight-ready/2026-06-29"`
                  : key === "persistence_check_ready"
                    ? `"${key}": "receipt://api-runtime/persistence-ready/2026-06-29"`
                    : key === "host_firewall_configured"
                      ? `"${key}": "receipt://api-runtime/firewall-configured/2026-06-29"`
                      : key === "tls_certificate_ready"
                        ? `"${key}": "receipt://api-runtime/tls-ready/2026-06-29"`
                      : key === "rollback_path_defined"
                        ? `"${key}": "site:ops/api-production-readiness-gate.md"`
                        : key === "private_runtime_witness_ready"
                          ? `"${key}": "control-plane:receipt/runtime-witness-ready-2026-06-29"`
                          : key === "dns_authority_ready"
                            ? `"${key}": "cloudflare:audit/dns-authority-2026-06-29"`
        : `"${key}": "missing"`;
  ...
  for (const term of [
    "function evaluateApiRuntimeManualEvidenceChecklist",
    "validatePublicSafeEvidenceRef",
    "manual_evidence_pass_ref_missing",
    "apiDnsPublicationAllowed: contractPass && allRowsPass",
    "raw_headers=not_recorded",
    "--require-ready",
  ]) {
    if (!apiRuntimeManualEvidenceValidator.includes(term)) {
      recordFailure(`api_runtime_manual_evidence_validator_term_missing:${term}`);
    }
  }
  for (const term of [
    ...
    if (!apiRuntimeManualEvidenceValidatorTest.includes(term)) {
      recordFailure(`api_runtime_manual_evidence_validator_test_term_missing:${term}`);
    }
  }
...
function validatePortfolioRouteContract() {
  ...
  const requiredTerms = [
    ...
    "Proof Lens",
    "Builder Inspect",
    "Script execution is unavailable in this browser context.",
    "portfolio-helper-fallback",
  ...
  const requiredCssPatterns = [
    ...
    ["portfolio_helper_fallback", /\.portfolio-helper-fallback\s*\{[^}]*max-width:\s*760px;[^}]*border-radius:\s*8px;/s],
