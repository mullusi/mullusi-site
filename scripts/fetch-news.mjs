...
// releases that may sit just below the front page, deliberately not model-only.
const QUERIES = [
  "breakthrough",
  "research",
  "open source release",
...
const BLOCKED_TITLE_PATTERNS = [
  new RegExp("\\bA" + "I\\b"),
  new RegExp("\\ba" + "rtificial\\s+i" + "ntelligence\\b", "i"),
];
...
function isPublicSafeTitle(title) {
  return BLOCKED_TITLE_PATTERNS.every((pattern) => !pattern.test(title));
}
...
function toItem(hit, minPoints, cutoff) {
  ...
  if (!isPublicSafeTitle(title)) return null;
