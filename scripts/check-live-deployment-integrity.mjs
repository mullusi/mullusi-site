...
export function evaluateDeploymentIntegrityEvidence(evidence) {
  ...
  if (liveStatus.site !== "mullusi.com") {
    hardFindings.push(`live_status_site_invalid:${liveStatus.site || ""}`);
  }
  if (liveStatus.public_state !== "Published") {
    hardFindings.push(`live_status_public_state_invalid:${liveStatus.public_state || ""}`);
  }

  for (const relativePath of governedHashPaths) {
    if (!liveHashes[relativePath]) {
      hardFindings.push(`live_status_hash_missing:${relativePath}`);
    }
  }
  ...
  for (const relativePath of Object.keys(liveHashes)) {
    ...
    if (!governedPathSet.has(relativePath)) {
      hardFindings.push(`live_status_hash_path_unexpected:${relativePath}`);
    }
  }
  ...
  if (!manifestHashesEqual(localHashes, liveHashes)) {
    softFindings.push("local_status_manifest_mismatch");
  }
...
async function runCli() {
  ...
  try {
    ...
    if (result.proofState === "Fail" || (requireLocalMatch && result.proofState !== "Pass")) {
      process.exit(1);
    }
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.log(`verdict=AwaitingEvidence\nproof_state=Unknown\nlive_deployment_integrity_state=AwaitingEvidence\nerror=${message}\nraw_response_bodies=not_recorded\nraw_response_headers=not_recorded`);
    if (!args.includes("--allow-pending")) {
      process.exit(1);
    }
  }
}
