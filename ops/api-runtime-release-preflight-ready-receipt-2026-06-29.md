<!--
Purpose: record a public-safe release preflight receipt for api.mullusi.com.
Governance scope: pre-DNS API runtime release validation, Render shell evidence, private-value exclusion, and DNS denial.
Dependencies: Render private service shell, scripts/preflight_release.py, and ops/api-runtime-manual-evidence-checklist.md.
Invariants: no secret values, host addresses, database URLs, provider account identifiers, DNS targets, raw provider payloads, or private recovery material are recorded.
-->

# API Runtime Release Preflight Ready Receipt

Receipt ref:

```text
receipt://api-runtime/release-preflight-ready/2026-06-29
```

Evidence statement:

```text
surface_id=api.mullusi.com
evidence_item=release_preflight_ready
state=Pass
source=render_shell:scripts/preflight_release.py
release_preflight=ready
required_environment=pass
image=pass
image_detail=tag:v2026.06.11-govern-cloud.1
runtime_key_presence=pass
database_url_shape=pass:remote_postgresql_configured
persistence_policy=pass:required
allowed_origins=pass:count_4
database_schema=pass:postgres_schema_ready
secret_values_recorded=false
host_addresses_recorded=false
database_urls_recorded=false
provider_values_recorded=false
dns_targets_recorded=false
raw_payloads_recorded=false
dns_publication_allowed=false
```

This receipt does not close firewall, TLS, rollback, private runtime witness, or
DNS authority gates.

STATUS:
  Completeness: 100%
  Self-attested invariants: public-safe status only, no runtime values recorded, DNS remains blocked
  Open issues: persistence receipt, firewall, TLS, rollback, private runtime witness, DNS authority
  Next action: collect persistence_check_ready evidence without recording private runtime values
